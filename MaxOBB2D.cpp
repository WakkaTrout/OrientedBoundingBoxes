                ...
                for ( int k = 0; k < num_points; ++k )
                {
                    for ( int l = k + 1; l < num_points; ++l )
                    {
                        ...
                        if ( Pkl.x != 0 || Pkl.y != 0 )
                        {
                            ...
                            // TODO: The below sign_term divides by a value that is not accounted for in previous checks. This implies that a) the max cannot occur at that orientation and must be culled or b) there is a special case not being handled (and should probably somehow be massaged into the overall structure). Look into this to see how to resolve it
